...
Route::get('/veritabani-kur', function () {
    // 1. Veritabanındaki tüm tabloları zorla siler
    Schema::disableForeignKeyConstraints();
    foreach (DB::select('SHOW TABLES') as $table) {
        $table_array = get_object_vars($table);
        Schema::drop(reset($table_array));
    }
    Schema::enableForeignKeyConstraints();

    // 2. Tabloları tertemiz baştan oluşturur
    \Illuminate\Support\Facades\Artisan::call('migrate', ['--force' => true]);
    
    // 3. Örnek verileri (seed) yükler
    \Illuminate\Support\Facades\Artisan::call('db:seed', ['--force' => true]);

    return 'İşlem tamam Ci! Veritabanını tamamen temizledim ve her şeyi baştan kurdum.';
});
